...
**首页就一处入口：四个岗位标签**（`ROLE_PATHS`），每个标签自带说明、进度条和「继续」按钮，
不再把「岗位路线 / 按目标选起点 / 学习路径」三块讲同一件事的内容堆在一页上。
全部 54 节的阶段目录收在标签下面一个默认折叠的「全部课程」块里，只露一行 `已完成 N/54`。
...
| **网络运维工程师**（网络工程师 / 数据中心网络） | 35 节 · 约 18 小时 | 覆盖最广的一条：L0 全部 → 代理/VPN/透明网关 → L1 基本整段（含 MetalLB、Ingress）→ RoCE/IB/perftest → 以太网与地址规划、可观测性与值班 |

四条线都从 L0 起步（术语与指标是共同地基），之后分叉。
覆盖最广的网络运维那条也仍是裁剪 —— 机内互联（PCIe / NVLink）、GPU 专项（NCCL、GDS）
与加速卸载专题（DPDK、eBPF/XDP、DPU、MPI）都不在上面。
没排进某条线的课不会消失，展开「全部课程」就能直接进去。
