...
**按岗位裁剪**：首页第一块是 `ROLE_PATHS` 定义的**岗位路线**。
54 节课不必都学，而"新手"这个身份又太笼统 —— 架构师要的是算账与选型，运维要的是排障手感，
必修课重叠不到一半。所以先选岗位，再看被裁剪过的清单，每条线还切成几段推进：

| 岗位 | 覆盖 | 重点 |
| --- | --- | --- |
| **解决方案架构师**（方案工程师 / 售前） | 18 节 · 约 9 小时 | 术语与数字 → 高性能网络的技术底牌 → L3 规划算账与选型 → 方案配套（K8s、GDS、NVMe-oF、DPU） |
| **计算集群运维工程师**（服务工程师 / 交付实施） | 23 节 · 约 12 小时 | L0 排障基本功 → SSH 远程 → K8s 容器网络 → PCIe/RoCE/perftest/NCCL 整条链路 → 值班 SOP；四个闯关全在这条线上 |
